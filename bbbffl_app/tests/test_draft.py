...
def domain(entries=4, limit=2, players=12):
    ...
    for number in range(entries):
        ...
        season_entries.append(identities.create_entry(season.season_id, f"licence-{number}", coach.coach_id, f"Team {number}"))
    ownership = OwnershipRepository(db)
    ownership.configure_squad_limit(season.season_id, limit)
    pool = PlayerPoolRepository(db)
    season_players = [pool.refresh_player(season.season_id, number + 1, f"Player {number}") for number in range(players)]
...
def test_execution_creates_ownership_and_allows_snake_consecutive_picks():
    ...
    draft.accept_order(season.season_id, [entry.season_entry_id for entry in entries])
    draft.execute_pick(season.season_id, entries[0].season_entry_id, players[0].season_player_id, completed_at="2027-01-01")
    draft.execute_pick(season.season_id, entries[1].season_entry_id, players[1].season_player_id, completed_at="2027-01-02")
    draft.execute_pick(season.season_id, entries[1].season_entry_id, players[2].season_player_id, completed_at="2027-01-03")
...
def test_traded_adjacent_pick_allows_same_owner_twice():
    ...
    assert [pick.current_season_entry_id for pick in draft.picks(season.season_id)[:2]] == [entries[0].season_entry_id] * 2
...
def test_capacity_and_completed_pick_rejections_are_atomic():
    ...
    with pytest.raises(DraftPickCompletedError):
        draft.execute_pick(season.season_id, entries[0].season_entry_id, players[1].season_player_id, pick_id=completed.draft_pick_id)
...
def test_completed_history_uses_durable_ids_after_names_and_roster_change():
    ...
    draft.accept_order(season.season_id, [entry.season_entry_id for entry in entries])
    completed = draft.execute_pick(season.season_id, entries[0].season_entry_id, players[0].season_player_id, completed_at="2027-01-01")
    with db.engine.begin() as conn:
        ...
        conn.exec_driver_sql("UPDATE season_player_pool SET display_name='Renamed player' WHERE season_player_id=?", (players[0].season_player_id,))
