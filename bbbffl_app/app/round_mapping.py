...
def _mapping(row) -> RoundMapping:
    return RoundMapping(row["mapping_id"], row["bbbffl_round_id"], row["revision"], row["state"], row["provider"], row["afl_season_id"], row["afl_round_id"], row["created_at"], row["created_by"], row["reason"])
...
class RoundMappingRepository:
    ...
    def propose(self, bbbffl_round_id: str, *, state: str = "unresolved", afl_season_id: int | None = None,
                afl_round_id: int | None = None, provider: str = "afl-api-v1",
                actor: ActorContext = ActorContext.anonymous_operator("admin"), reason: str | None = None) -> RoundMapping:
        ...
        with transaction(self.database) as conn:
            head = conn.execute("SELECT * FROM round_afl_mapping WHERE bbbffl_round_id=?" + _for_update_suffix(self.database), (bbbffl_round_id,)).fetchone()
            if head:
    ...
    def correct(self, bbbffl_round_id: str, afl_season_id: int, afl_round_id: int, validator: AflReferenceValidator, *, reason: str, actor: ActorContext = ActorContext.anonymous_operator("admin"), provider: str = "afl-api-v1") -> RoundMapping:
        if not reason:
    ...
    def resolve(self, bbbffl_round_id: str) -> RoundMapping | None:
        row = self.database.execute(self._select() + " WHERE m.bbbffl_round_id=? AND r.state='accepted'", (bbbffl_round_id,)).fetchone()
        return _mapping(row) if row else None

    def history(self, bbbffl_round_id: str) -> list[RoundMapping]:
        rows = self.database.execute(self._select(False) + " WHERE m.bbbffl_round_id=? ORDER BY r.revision", (bbbffl_round_id,)).fetchall()
        return [_mapping(row) for row in rows]

    def _activate(self, round_id, season, afl_round, provider, actor, reason, correction):
        with transaction(self.database) as conn:
            head = conn.execute("SELECT * FROM round_afl_mapping WHERE bbbffl_round_id=?" + _for_update_suffix(self.database), (round_id,)).fetchone()
            if not head:
                if correction:
                ...
                if correction != (old["state"] == "accepted"):
                    raise ValueError("use correction for an accepted mapping" if old["state"] == "accepted" else "correction requires an accepted mapping")
                revision = head["current_revision"] + 1
                before = {"state": old["state"], "afl_season_id": old["afl_season_id"], "afl_round_id": old["afl_round_id"]}
                conn.execute("UPDATE round_afl_mapping SET current_revision=? WHERE mapping_id=?", (revision, mapping_id))
            ...
            append_event(conn, actor=actor, action=MAPPING_CORRECTED if correction else MAPPING_ACCEPTED, entity_type="round.afl_mapping", entity_id=mapping_id, entity_version=str(revision), before_state=before, after_state=after, reason=reason)
            return self._get(conn, mapping_id)

    @staticmethod
    def _insert_revision(conn, mapping_id, revision, state, provider, season, afl_round, actor, reason):
        conn.execute("INSERT INTO round_afl_mapping_revision VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (mapping_id, revision, state, provider, season, afl_round, _now(), actor.actor_id, reason))
    ...
    def _select(current=True):
        ...
        return "SELECT m.bbbffl_round_id, r.* FROM round_afl_mapping m JOIN round_afl_mapping_revision r ON r.mapping_id=m.mapping_id" + join

    def _get(self, conn, mapping_id):
