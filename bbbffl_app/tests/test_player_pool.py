...
def setup_domain(limit=2):
    ...
    entries = [
        identities.create_entry(
            season.season_id, f"licence-{i}", coach.coach_id, f"Team {i}"
        )
...
def test_release_transfer_and_reacquisition_preserve_non_overlapping_history():
    ...
    ownership.transfer(
        player.season_player_id, entries[1].season_entry_id, effective_at="2027-02-01"
    )
    ownership.release(player.season_player_id, effective_at="2027-03-01")
    ownership.acquire(
        player.season_player_id, entries[0].season_entry_id, effective_at="2027-04-01"
    )
    ...
    assert (
        ownership.owner_at(player.season_player_id, "2027-02-15").season_entry_id
...
def test_canonical_player_is_independent_between_replay_and_live_seasons():
    ...
    old = pool.refresh_player(
        replay.season_id, 396, "Nick Daicos", afl_team_name="Collingwood 2026"
    )
    new = pool.refresh_player(
        live.season_id, 396, "Nick Daicos", afl_team_name="Collingwood 2027"
    )
    assert old.season_player_id != new.season_player_id
    assert pool.get(replay.season_id, 396).afl_team_name == "Collingwood 2026"
    assert [p.canonical_player_id for p in pool.list_selectable(live.season_id)] == [
        396
    ]
...
def test_squad_capacity_rejects_excess_player():
    ...
    ownership.acquire(
        first.season_player_id, entries[0].season_entry_id, effective_at="2027-01-01"
    )
...
def test_afl_club_refresh_does_not_change_ownership_or_audit_history():
    ...
    ownership.acquire(
        player.season_player_id, entries[0].season_entry_id, effective_at="2027-02-01"
    )
    before = ownership.history(player.season_player_id)
    event_count = len(
        AuditEventRepository(db).list_events(entity_type="ownership.period")
    )
    ...
    assert (
        len(AuditEventRepository(db).list_events(entity_type="ownership.period"))
        == event_count
    )
