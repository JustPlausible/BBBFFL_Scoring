...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["original_season_entry_id", "season_id"],
            ["season_entry.season_entry_id", "season_entry.season_id"],
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["current_season_entry_id", "season_id"],
            ["season_entry.season_entry_id", "season_entry.season_id"],
            ondelete="RESTRICT",
        ),
        sa.ForeignKeyConstraint(
            ["selected_season_player_id", "season_id"],
            ["season_player_pool.season_player_id", "season_player_pool.season_id"],
            ondelete="RESTRICT",
        ),
        sa.CheckConstraint(
            "overall_number > 0 AND draft_round > 0 AND round_position > 0", name="ck_draft_pick_numbers_positive"
        ),
        sa.CheckConstraint(
            "(selected_season_player_id IS NULL) = (completed_at IS NULL)", name="ck_draft_pick_completion"
        ),
    )
    op.create_table(
        "draft_pick_transfer",
        ...
        sa.Column(
            "draft_pick_id", sa.Text(), sa.ForeignKey("draft_pick.draft_pick_id", ondelete="RESTRICT"), nullable=False
        ),
        sa.Column(
            "from_season_entry_id",
            sa.Text(),
    ...
    if bind.dialect.name == "sqlite":
        op.execute(
            "CREATE TRIGGER draft_order_immutable_update BEFORE UPDATE ON draft_order_position BEGIN SELECT RAISE(ABORT, 'accepted draft order is immutable'); END"
        )
        op.execute(
            "CREATE TRIGGER draft_order_immutable_delete BEFORE DELETE ON draft_order_position BEGIN SELECT RAISE(ABORT, 'accepted draft order is immutable'); END"
        )
        op.execute(
        ...
        """)
        op.execute(
            "CREATE TRIGGER draft_order_immutable_update BEFORE UPDATE OR DELETE ON draft_order_position FOR EACH ROW EXECUTE FUNCTION reject_immutable_draft_change()"
        )
        op.execute(
            "CREATE TRIGGER completed_pick_immutable BEFORE UPDATE OF selected_season_player_id, completed_at ON draft_pick FOR EACH ROW WHEN (OLD.completed_at IS NOT NULL) EXECUTE FUNCTION reject_immutable_draft_change()"
        )
        op.execute(
            "CREATE TRIGGER completed_pick_delete_immutable BEFORE DELETE ON draft_pick FOR EACH ROW WHEN (OLD.completed_at IS NOT NULL) EXECUTE FUNCTION reject_immutable_draft_change()"
        )
...
def downgrade():
    ...
    if bind.dialect.name == "sqlite":
        for trigger in (
            "completed_pick_delete_immutable",
            "completed_pick_immutable",
            "draft_order_immutable_delete",
            "draft_order_immutable_update",
        ):
            op.execute(f"DROP TRIGGER IF EXISTS {trigger}")
    elif bind.dialect.name == "postgresql":
        op.execute("DROP TRIGGER IF EXISTS completed_pick_delete_immutable ON draft_pick")
        op.execute("DROP TRIGGER IF EXISTS completed_pick_immutable ON draft_pick")
        op.execute("DROP TRIGGER IF EXISTS draft_order_immutable_update ON draft_order_position")
