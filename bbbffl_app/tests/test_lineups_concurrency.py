...
def test_same_draft_revision_has_one_winner(postgres_url):
    ...
    results = race(
        [
            lambda: save(repo, round_, entries, scope, {"F1": player.season_player_id}, draft.revision),
            lambda: save(repo, round_, entries, scope, {"M1": player.season_player_id}, draft.revision),
        ]
    )
...
def test_same_submission_version_has_one_winner(postgres_url):
    ...
    results = race(
        [
            lambda: repo.submit(draft.lineup_id, expected_draft_revision=1, expected_submission_version=0),
            lambda: repo.submit(draft.lineup_id, expected_draft_revision=1, expected_submission_version=0),
        ]
    )
    assert sum(result == "conflict" for result in results) == 1
