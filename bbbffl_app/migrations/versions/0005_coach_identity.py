...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("season_entry_id", sa.Text(), sa.ForeignKey("season_entry.season_entry_id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_index("uq_entry_current_coach", "season_entry_coach_history", ["season_entry_id"], unique=True, sqlite_where=sa.text("ended_at IS NULL"), postgresql_where=sa.text("ended_at IS NULL"))
    op.create_table(
    ...
    op.create_index("uq_entry_current_team_name", "season_entry_team_name_history", ["season_entry_id"], unique=True, sqlite_where=sa.text("ended_at IS NULL"), postgresql_where=sa.text("ended_at IS NULL"))


def downgrade():
    bind = op.get_bind()
    counts = sum(bind.execute(sa.text(f"SELECT COUNT(*) FROM {table}")).scalar_one() for table in ("coach", "season_entry"))
    if counts:
