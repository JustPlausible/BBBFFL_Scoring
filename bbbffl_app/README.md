...
| Legacy Grand Final (prototype) | `GET /legacy/grand-final`, `GET /api/public/state`, `GET /admin`, `POST /api/admin/*` | Deliberately separate retained prototype -- see "What the legacy Grand Final prototype still does not do" above. No coach authentication, no fixtures/ladder; mutations are gated by the shared `X-Admin-Token`. |
