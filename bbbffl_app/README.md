...
| Administrator Dashboard | `GET /admin/dashboard`, `GET /api/admin/dashboard` | Regular season (issue #148). The Administrator's role home: durable season/platform governance -- identity/role/environment, a multi-season portfolio, setup/preseason readiness (round *definitions* kept distinct from opened lifecycle rows), a categorised governance attention queue, a state-aware workflow map, a concise Scorer operational handoff summary, a role/access overview and an audit/integrity overview -- never a second Scorer attention queue or workflow engine. Strictly Administrator-only (unlike the Scorer Dashboard). `/account` links here for any signed-in identity holding Administrator authority. See [`../docs/admin-dashboard.md`](../docs/admin-dashboard.md). |
| Scorer Operations Dashboard | `GET /scorer`, `GET /scorer/dashboard` (alias), `GET /api/scorer/dashboard` | Regular season (issue #147). The Scorer's normal operational home: one authoritative read model over current season/round/lifecycle, a deterministic next safe action, a categorised attention queue, all ten teams' lineup readiness, lockout/trigger state, review/publication readiness and recent audited activity -- aggregation/navigation only, every item links to its owning workflow below. `/account` links here for any signed-in identity holding Scorer/Replay-Operator/Administrator authority. See [`../docs/scorer-dashboard.md`](../docs/scorer-dashboard.md). |
...
| Scorer Season Centre | `GET /admin/season-centre`, `GET /admin/season-centre/{season_id}`, `POST /api/admin/season-centre/...` | Regular season (issue #100). Establish/inspect one season's identity, coaches and season entries -- real coach names and public BBBFFL team names, never `Team A`/`Player 1` placeholders -- plus a readiness summary and links to draft/Round Centre once reachable. See [`../docs/season-centre.md`](../docs/season-centre.md). Requires the strict admin authority (not merely `scorer`). |
| Season setup | `GET /admin/season-setup/{season_id}`, `/api/admin/season-setup/{season_id}/...` | Live-season initialization (issue #237): player pool from live afl-api, ordinary competition and Rounds 1-N, Opening Round compensating-bye rules (only if the AFL fixture has an Opening Round), squad limit and preseason draft order, then Finals (from the live ladder, once every regular-season round is final) and SuperScore (once the Finals bracket exists). Scorer, Secretary or Administrator. The supported production path -- the 2026 `scripts/*_2026*.py` bootstrap/replay tools refuse to run in production. See [`../docs/season-setup.md`](../docs/season-setup.md). |
| Public regular-season entry / Round Centre | `GET /`, `GET /seasons/{season_id}`, `GET /seasons/{season_id}/rounds/{round_id}`, `GET /api/public/seasons/{season_id}/rounds/{round_id}`, `.../ladder` | Primary application. `/` selects the latest persisted ordinary season without requiring its UUID; the explicit season URL enters that season's Round Centre. With no bootstrapped ordinary rounds, `/` shows a useful empty state. The anonymous read model uses the same authoritative round/official-result/ladder state -- never scoring/ladder logic of its own (`app/public_rounds.py`). |
...
| Legacy Grand Final (prototype) | `GET /legacy/grand-final`, `GET /api/public/state`, `GET /admin`, `POST /api/admin/*` | Deliberately separate retained prototype -- see "What the legacy Grand Final prototype still does not do" above. No coach authentication, no fixtures/ladder; mutations are gated by the shared `X-Admin-Token`. `GET /admin` is this legacy scoring panel, not the Administrator Dashboard above (`GET /admin/dashboard`) -- issue #148 deliberately keeps the two routes separate rather than merging legacy token authority with session-native Administrator authority; see [`../docs/admin-dashboard.md`](../docs/admin-dashboard.md#route-admindashboard-not-admin). |
| SuperScore (opt-in) | `GET /superscore`, `GET /admin/superscore`, `POST /api/*/superscore/*` | Independent application surface -- 404s entirely unless `BBBFFL_SUPERSCORE_CONFIG_PATH` is set. |
