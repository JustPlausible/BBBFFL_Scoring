...
Then, in the browser: sign in as the provisioned Administrator, open **Draft Board** and confirm the next action is **Pick 1**. From **Season Centre**, the Opening Round Operations section is now visible (accepted rules exist); its readiness indicator counts explicit per-entry submission *confirmations*, never eligible clubs/players or nomination counts, so with no picks made and no confirmations yet it correctly reads `0/10 teams confirmed` (not yet ready -- every one of the ten entries must still explicitly confirm, even the ones that end up nominating nothing). Open it from that link and confirm all ten accepted rules are visible with zero nominations. Player nominations, and each entry's explicit confirmation, are entered later, through that same Opening Round Operations workflow reached from Season Centre, only after the draft has produced actual owned players (see section F, steps 6–7) -- never during bootstrap and never inferred from later results.
...
validate/freeze opening squads is steps 3–5; (4) enter zero or more Opening
Round reconstructed nominations for each represented entry is step 6; (5)
explicitly confirm that entry's Opening Round submission, repeating until
every entry has confirmed, is step 7; (6) prepare/open applicable ordinary
BBBFFL rounds is section G, next.
...
6. **Enter Opening Round reconstructed nominations.** From **Season Centre**, open the season, and once the draft has produced owned players use the **Opening Round Operations** action shown there (it links to `/operations/seasons/<season-id>/opening-round`; there is no need to know or type that URL directly). For each represented entry, for every owned player from a club covered by an accepted Opening Round rule that the historical record supports nominating, choose that player -- the page resolves the matching club rule, compensating AFL round and target BBBFFL round automatically -- pick the target slot, and create the nomination with a replay/reconstructed reason. **A zero or partial nomination set is valid and expected: owning an eligible player never itself requires a nomination for that club.** Nominations are operator/replay evidence entered from the acquired Opening Round club/bye facts (section C) and the coach's actual drafted squad -- never inferred, guessed, or backfilled from any later AFL statistics or match results.
7. **Explicitly confirm each entry's Opening Round submission.** Still on the Opening Round Operations page for each represented entry in turn, use the **Confirm Opening Round submission** action -- valid, and expected, even for an entry with zero nominations. Confirmation records the authenticated replay operator's provenance and confirmation time (never the historical coach), and once confirmed the entry's nominations are locked: a further change requires an explicit, audited reopen first. Repeat for every represented entry, then verify Season Centre (or the Opening Round Operations page) reads `10/10 teams confirmed` and ready before continuing -- any entry still unconfirmed is a stop-and-fix condition, not a warning to note and proceed past.
...
5. Open **Round Preflight**; verify five fixtures, accepted AFL mapping, and selective/main trigger configuration. Where an accepted Opening Round rule targets this round, Round Preflight itself blocks opening and links back to **Opening Round Operations** if any entry has not yet explicitly confirmed its Opening Round submission (section F, steps 6–7 should already have completed this; treat a blocker here as a sign that step was missed, not something to work around), or if a separate nomination integrity conflict exists.
...
8. Repeat the checkpoint command at each relevant scheduled match start and main boundary, restarting app each time. Confirm later-club edits remain possible and started/triggered edits are rejected. **Weekly Lineup** (`/operations/rounds/<round-id>/lineup`) is itself an authoritative lock-state inspection surface, not only a draft editor (issue #138): it renders the same `editable`/`locked`/`indeterminate` read model as the coach's own lineup page for every represented team, with the covering selective/main trigger (or Opening Round deferred nomination, shown as its own distinct lock type) and effective lock time named per position. A checkpoint's AFL match evidence commonly still reads `UPCOMING` even once a trigger has activated because replay time reached its scheduled start (`activation_reason: match_time_reached`, shown in the page's "Lockout plan" panel) -- this is expected and is not itself a defect; treat a position that still renders as an editable dropdown *after* its covering trigger has activated as the actual defect, since the underlying submission guard would reject the edit regardless of what the page displays.
...
**A submitted lineup recorded in the wrong position after its covering lockout has already activated** (issue #137 -- e.g. a coach named a player at Tackler in league chat, the scorer entered Interchange, and the selective/main trigger for that player's AFL match fired before the error was noticed) is the one documented exception: it is corrected through **Locked Lineup Correction** (`/scorer/lineup-correction`, `lineup.correct_locked` capability -- granted to Replay Operator only for the seasons this replay's role grants actually cover), never by ad-hoc SQL, a checkpoint rollback, or a score-only override, none of which preserve the immutable submission/lock history this playbook otherwise depends on. The correction requires a substantive reason (record the league-chat consultation or scorer transcription explanation), creates a new authoritative submission version while preserving the original version and lock evidence unchanged, and is fully audited (`app.audit.LINEUP_CORRECTED`) -- log it in the replay test log's Notes column the same as any other operator action. It is refused outright once a round has reached final publication; a discrepancy discovered after that point is the separate official-result correction workflow (`docs/scorer-round-review.md`), not this one.
...
- [ ] Opening Round deferred-selection and compensating-bye provenance is complete, every represented entry explicitly confirmed its Opening Round submission (including any with zero or a partial nomination set), and Opening Round Operations' confirmation readiness (Season Centre and Round Preflight, `10/10 teams confirmed`) showed ready before every dependent round was opened.
