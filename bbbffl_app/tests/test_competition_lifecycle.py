...
def configured(database, year, afl_round=100):
    ...
    logical_round = seasons.create_round(
        competition.competition_id, "round-1", "Round 1", 1
    )
    ...
    for number in range(10):
        ...
        entries.append(
            identities.create_entry(
                season.season_id, f"licence-{number}", coach.coach_id, f"Team {number}"
            )
        )
    ...
    RoundMappingRepository(database).accept(
        logical_round.bbbffl_round_id, year, afl_round, KnownRound(year, afl_round)
    )
        ...
        len(
            {
                ...
                for entry in (match.home_season_entry_id, match.away_season_entry_id)
            }
        )
        == 10
    )
...
def test_creation_fails_closed_for_nonaccepted_mapping(mapping_state):
    ...
    fixtures.save_draft(season.season_id, [entry.season_entry_id for entry in entries])
    fixtures.freeze(season.season_id)
    RoundMappingRepository(database).propose(
        round_.bbbffl_round_id, state=mapping_state
    )
    with pytest.raises(ValueError, match="accepted"):
        CompetitionLifecycleRepository(database).create_ordinary_round(
            round_.bbbffl_round_id
        )
    assert (
        CompetitionLifecycleRepository(database).get_round(round_.bbbffl_round_id)
        is None
    )
...
def test_renames_do_not_change_historical_matchup_identity():
    ...
    identities.transfer_entry(
        entries[1].season_entry_id, identities.create_coach("Replacement").coach_id
    )
...
def test_first_finalisation_and_correction_preserve_immutable_versions():
    ...
    assert [
        result.version for result in lifecycle.result_history(matchup.matchup_id)
    ] == [1]
    assert (
        lifecycle.effective_result(matchup.matchup_id).home_score
        == first[matchup.matchup_id][0]
    )
    corrected = scores(lifecycle, round_.bbbffl_round_id, 10)
    lifecycle.correct_results(
        round_.bbbffl_round_id, corrected, reason="authorised transcription correction"
    )
    ...
    events = AuditEventRepository(database).list_events(
        entity_type="competition.matchup", entity_id=matchup.matchup_id
    )
...
def test_simulated_failure_rolls_back_all_five_results_and_final_state():
    ...
    assert lifecycle.get_round(round_.bbbffl_round_id).state == "review"
    assert all(
        lifecycle.result_history(match.matchup_id) == []
        and match.effective_official_version is None
...
def test_replay_and_live_round_results_are_independent():
    ...
    replay.publish_results(
        replay_round.bbbffl_round_id, scores(replay, replay_round.bbbffl_round_id)
    )
    ...
    assert all(
        live.effective_result(match.matchup_id) is None
        for match in live.list_matchups(live_round.bbbffl_round_id)
    )
