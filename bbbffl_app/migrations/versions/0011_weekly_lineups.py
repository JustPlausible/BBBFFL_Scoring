...
def upgrade():
    op.create_table(
        ...
        sa.UniqueConstraint("season_id", "competition_id", "bbbffl_round_id", "season_entry_id", name="uq_weekly_lineup_scope"),
        ...
        sa.CheckConstraint("effective_submission_version IS NULL OR effective_submission_version >= 1", name="ck_lineup_effective_version"),
    )
    op.create_table(
        ...
        sa.Column("season_player_id", sa.Text(), sa.ForeignKey("season_player_pool.season_player_id", ondelete="RESTRICT")),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("source_type IN ('coach','scorer_proxy','carry_forward','system_derived')", name="ck_submission_source"),
    )
    op.create_table(
        ...
        sa.Column("position", sa.Text(), nullable=False),
        sa.Column("season_player_id", sa.Text(), sa.ForeignKey("season_player_pool.season_player_id", ondelete="RESTRICT")),
        sa.ForeignKeyConstraint(["lineup_id", "version"], ["weekly_lineup_submission.lineup_id", "weekly_lineup_submission.version"], ondelete="RESTRICT"),
    ...
    if bind.dialect.name == "sqlite":
        for table in ("weekly_lineup_submission", "weekly_lineup_submission_slot"):
            op.execute(f"CREATE TRIGGER {table}_no_update BEFORE UPDATE ON {table} BEGIN SELECT RAISE(ABORT, 'submitted lineups are immutable'); END")
            op.execute(f"CREATE TRIGGER {table}_no_delete BEFORE DELETE ON {table} BEGIN SELECT RAISE(ABORT, 'submitted lineups are immutable'); END")
    elif bind.dialect.name == "postgresql":
        op.execute("CREATE FUNCTION reject_submitted_lineup_mutation() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'submitted lineups are immutable'; END; $$ LANGUAGE plpgsql")
        for table in ("weekly_lineup_submission", "weekly_lineup_submission_slot"):
            op.execute(f"CREATE TRIGGER {table}_immutable BEFORE UPDATE OR DELETE ON {table} FOR EACH ROW EXECUTE FUNCTION reject_submitted_lineup_mutation()")
...
def downgrade():
    ...
    for table in ("weekly_lineup_submission_slot", "weekly_lineup_submission", "weekly_lineup_draft_slot", "weekly_lineup"):
        op.drop_table(table)
