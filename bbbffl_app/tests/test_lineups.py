...
def test_draft_repeated_edits_conflict_and_survive_new_repository():
    ...
    edited = save(
        repo, round_, entries, scope, {"F1": players[0].season_player_id, "F2": players[1].season_player_id}, 1
    )
    reordered = save(
        repo, round_, entries, scope, {"F1": players[1].season_player_id, "F2": players[0].season_player_id}, 2
    )
    assert (
        WeeklyLineupRepository(db).get_draft(
...
def test_submission_is_immutable_resubmission_history_and_ownership_history_survives():
    ...
    save(repo, round_, entries, scope, {"M1": players[1].season_player_id}, 1)
