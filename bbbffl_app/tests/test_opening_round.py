...
def test_resolve_afl_club_name_is_none_until_a_player_pool_row_caches_it():
    ...
    own_named_player(db, scope["season_id"], entries[0], 920100, "GWS Player", afl_team_id=15, afl_team_name="GWS Giants")
...
def test_describe_accepted_rule_produces_human_readable_primary_label_with_ids_retained():
    ...
    own_named_player(db, scope["season_id"], entries[0], 920101, "GWS Player", afl_team_id=15, afl_team_name="GWS Giants")
    client = RoundNumberClient({ev.afl_season_id: [type("R", (), {"round_id": 1343, "round_number": 0})(), type("R", (), {"round_id": 1347, "round_number": 4})()]})
...
def test_readiness_transitions_from_missing_to_complete_for_one_eligible_entry():
    ...
    ev = evidence.EVIDENCE_2026
    rule = accept_rule(db, scope["season_id"], 15, ev, ev.compensating_bye_round["GWS"], round_.bbbffl_round_id)
    player = own_named_player(db, scope["season_id"], entries[0], 920102, "GWS Player", afl_team_id=15, afl_team_name="GWS Giants")
    ...
    or_client = MultiRoundMatchClient({ev.afl_opening_round_id: [Match(9101, Team(15, "GWS"), Team(1, "ADEL"), "CONCLUDED")]})
    OpeningRoundNominationRepository(db).nominate(
...
def test_readiness_reports_each_entry_independently_across_multiple_rules():
    ...
    rule_carl = accept_rule(db, scope["season_id"], 5, ev, ev.compensating_bye_round["CARL"], round_.bbbffl_round_id)
    bl_player = own_named_player(db, scope["season_id"], entries[0], 920103, "BL Player", afl_team_id=2, afl_team_name="Brisbane Lions")
...
def test_readiness_detects_mismatched_and_conflicting_nominations_defensively():
    ...
    ev = evidence.EVIDENCE_2026
    rule = accept_rule(db, scope["season_id"], 15, ev, ev.compensating_bye_round["GWS"], round_.bbbffl_round_id)
    player = own_named_player(db, scope["season_id"], entries[0], 920105, "GWS Player", afl_team_id=15, afl_team_name="GWS Giants")
    or_client = MultiRoundMatchClient({ev.afl_opening_round_id: [Match(9103, Team(15, "GWS"), Team(1, "ADEL"), "CONCLUDED")]})
    nomination = OpeningRoundNominationRepository(db).nominate(
        rule.rule_id, entries[0].season_entry_id, "M1", player.season_player_id, or_client, actor=SCORER
    )
    ...
    with database_transaction(db) as conn:
        conn.execute("UPDATE season_player_pool SET afl_team_id=? WHERE season_player_id=?", (99, player.season_player_id))
    ...
    with database_transaction(db) as conn:
        conn.execute("UPDATE season_player_pool SET afl_team_id=? WHERE season_player_id=?", (15, player.season_player_id))
