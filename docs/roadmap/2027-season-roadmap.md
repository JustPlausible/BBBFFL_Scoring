...
| 13 | **Implemented:** draft order, picks and pick ownership | Frozen complete draw, stable materialised snake picks, independently tradeable ownership with audit, and transactional selection through the player ownership ledger. Completed selections are immutable; correction is added by package 14. See [draft ledger](../draft-ledger.md). | P1 / L | After 09–12; establishes selection ledger before UI. | Property tests across entry/squad sizes; consecutive turnaround/traded turns, atomic failures, durable history and concurrency-safe execution. | Maybe | Yes | No |
| 14 | **Implemented:** scorer-operated preseason draft workflow | Live board, proxy picks, validation, undo/correction with audit, pause/resume and explicit finalise. See [scorer draft workflow](../scorer-draft-workflow.md). | P1 / L | After 13; operational surface can initially be scorer-first. | API/UI workflow and concurrency tests; full synthetic ten-team draft resumes and finalises. | No | Yes | No |
| 15 | **Implemented:** finalise opening squads and preseason trades | Explicit preseason transaction/finalisation window, atomic multi-club player trades and window close; freezes a versioned opening-ownership snapshot Round 1 relies on. See [preseason-trades.md](../preseason-trades.md). | P1 / M | After 11 and 14; closes preseason state used by Round 1. | Atomicity/size/window/audit tests; opening squads all valid and closed edits rejected. | Yes | Yes | No |
