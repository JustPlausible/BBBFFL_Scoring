...
def test_diagnostic_flags_a_later_player_row_missing_a_scored_stat_field():
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        routes = {
            "/api/v1": DISCOVERY, "/api/v1/seasons": SEASONS,
            "/api/v1/seasons/85/rounds": ROUNDS_85, "/api/v1/seasons/84/rounds": ROUNDS_84,
            "/api/v1/rounds/1/matches": MATCHES, "/api/v1/matches/100": MATCH_100,
        }
...
def test_diagnostic_selects_the_round_matching_current_round_number_not_the_first_round():
    rounds = {
        "rounds": [
            {"round_id": 10, "season_id": 85, "round_number": 1, "name": "Round 1",
             "abbreviation": "R1", "start_time": None, "end_time": None, "byes": []},
            {"round_id": 99, "season_id": 85, "round_number": 5, "name": "Round 5",
             "abbreviation": "R5", "start_time": None, "end_time": None, "byes": []},
        ]
    }
    ...
    matches_for_current_round = {
        "matches": [
            {"match_id": 200, "round_id": 99, "season_id": 85, "status": "CONCLUDED",
             "start_time_utc": None, "home_team": {"team_id": 1, "name": "Home"},
             "away_team": {"team_id": 2, "name": "Away"}, "score_home": 50, "score_away": 40},
        ]
    }
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path == "/api/v1/rounds/10/matches":
            ...
            return httpx.Response(200, json={"matches": [{
                "match_id": 999, "round_id": 10, "season_id": 85, "status": "UPCOMING",
                "start_time_utc": None, "home_team": {"team_id": 3, "name": "Third"},
                "away_team": {"team_id": 4, "name": "Fourth"}, "score_home": None, "score_away": None,
            }]})
...
def test_diagnostic_treats_a_required_skip_as_not_fully_validated():
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path == "/api/v1/players/999999999999":
            return httpx.Response(404, json=ERROR_404)
        routes = {
            "/api/v1": DISCOVERY, "/api/v1/seasons/85/rounds": ROUNDS_85,
            "/api/v1/rounds/1/matches": MATCHES, "/api/v1/matches/100": MATCH_100,
            "/api/v1/matches/100/player-stats": PLAYER_STATS_100, "/api/v1/players/1": PLAYER_1,
            "/api/v1/players/1/seasons": PLAYER_1_SEASONS, "/api/v1/injuries": INJURIES,
            "/api/v1/matches/100/rosters": ROSTERS_100,
        }
        if path == "/api/v1/players" and request.url.params.get("search") == "":
            return httpx.Response(422, json=ERROR_422)
