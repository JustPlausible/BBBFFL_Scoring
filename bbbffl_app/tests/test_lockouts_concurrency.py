...
def match():
    return Match(
        match_id=70001, home_team=EARLY_HOME, away_team=EARLY_AWAY, status="UPCOMING", start_time_utc=START.isoformat()
    )
...
def postgres_context(url, year):
    ...
    incumbent = pool.refresh_player(
        scope["season_id"], year * 1000, "Incumbent", afl_team_id=EARLY_HOME.team_id, afl_team_name=EARLY_HOME.name
    )
    ownership.acquire(incumbent.season_player_id, entries[0].season_entry_id)
    challenger = pool.refresh_player(
        scope["season_id"], year * 1000 + 1, "Challenger", afl_team_id=EARLY_HOME.team_id, afl_team_name=EARLY_HOME.name
    )
    ownership.acquire(challenger.season_player_id, entries[0].season_entry_id)
    LockoutTriggerRepository(db).create(
        round_.bbbffl_round_id, "main", "main", 1, [match().match_id], reason="concurrency fixture"
    )
...
def test_concurrent_first_observations_of_a_lock_converge_to_one_record(postgres_url):
    ...
    rows = db.execute(
        "SELECT * FROM weekly_lineup_lock WHERE lineup_id=? AND position='F1'", (draft.lineup_id,)
    ).fetchall()
    assert len(rows) == 1
...
def test_lock_guard_does_not_disturb_ordinary_submission_serialization(postgres_url):
    ...
    def attempt(player):
        try:
            ...
            lineups.submit(
                edit.lineup_id, expected_draft_revision=edit.revision, expected_submission_version=0, lock_guard=guard
            )
            return "committed"
    ...
    def do_replace():
        try:
            triggers.replace(
                ...
                trigger_type="main",
                sequence=1,
                afl_match_ids=[match().match_id],
                reason="racing reconfiguration",
            )
            return "replaced"
