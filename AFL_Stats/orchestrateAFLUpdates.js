...
function monitorMatchStatus() {
  ...
  //Logger.log("🔄 Updating Game Schedule...");
  //fetchAFLGameSchedule();  // ✅ Refresh status before processing

  //const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Game Schedule");
  //if (!sheet) {
  //  Logger.log("❌ Error: 'Game Schedule' sheet not found.");
  //  return;
  //}
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const localTime = new Date(data[i][2]);
    const round = Number(data[i][3]);
    ...
    if (!currentRound && ["NS", ...isLiveStatus, "FT"].includes(status)) {
      const gameDay = new Date(data[i][1]);
